     ...
     public function createView()
     {
      ...
      public function update(Request $request)
      {
        ...
        public function daily_bounes(){
            $percentages = Percentage::get(['current_dat','created_at','amount']);
            $days=[];
            $day_percentages=[];
            $bonusdata=[];
            foreach($percentages as $percentage){
                //**GETTING CURRENT DATE ACCORDING PERCENTAGE */
                array_push($days,$percentage->current_dat);
               //**GETTING ALL DEPOSIT ACCORDING TO DATE OF PERCHENTAGE */
                $total_deposit = Deposit::whereDate('created_at','<',$percentage->current_dat)->where('user_id',Auth::user()->id)->get(['amount'])->sum('amount');
                // percentage -> 36.36 -> 
            ...
            for($i=0;$i < count($days);$i++){
                $day=explode(" ",$days[$i]);
                $bonusdata += [$day[0] => $day_percentages[$i]];
            }
           // return $bonusdata;
