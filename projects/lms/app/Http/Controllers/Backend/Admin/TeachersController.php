...
//        $request = $this->saveFiles($request);
        ...
        if ($request->passport) {
            $teacher->passport = $request->passport->store('/avatars', 'public');
        }
        if ($request->birthcertificate) {
            $teacher->birthcertificate = $request->birthcertificate->store('/avatars', 'public');
        }
        if ($request->citizenship) {
            $teacher->citizenship = $request->citizenship->store('/avatars', 'public');
        }
        if ($request->drivinglicence) {
            $teacher->drivinglicence = $request->drivinglicence->store('/avatars', 'public');
        }
        if ($request->centrallinkcard) {
            $teacher->centrallinkcard = $request->centrallinkcard->store('/avatars', 'public');
        }
        if ($request->pseid) {
            $teacher->pseid = $request->pseid->store('/avatars', 'public');
        }
        if ($request->securitylicence) {
            $teacher->securitylicence = $request->securitylicence->store('/avatars', 'public');
        }
        if ($request->tertiaryid) {
            $teacher->tertiaryid = $request->tertiaryid->store('/avatars', 'public');
        }
        if ($request->proofage) {
            $teacher->proofage = $request->proofage->store('/avatars', 'public');
        }
        if ($request->medicare) {
            $teacher->medicare = $request->medicare->store('/avatars', 'public');
        }
        if ($request->others) {
            $teacher->others = $request->others->store('/avatars', 'public');
        }
        ...
        $data = [
            ...
            'centrallinkcard'   => request()->centrallinkcard,
            'pseid'             => request()->pseid,
            'securitylicence'   => request()->securitylicence,
            'tertiaryid'        => request()->tertiaryid,
            'proofage'          => request()->proofage,
