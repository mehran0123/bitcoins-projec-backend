...
$('#reset-password').click(function(){
 ...
 if(!password){
    $('#message').html('<span class="text-danger text-center">Password is required*</span>');
  }else if(password.length <= 5){
    $('#message').html('<span class="text-danger text-center">Password length should be greater than 5 characters</span>');
  }else if(password != conf_password){
    $('#message').html('<span class="text-danger text-center">Passwords not matched</span>');
  }else{
    $('#message').html('');
    ...
    $.ajax({
                ...
                if(res == "true"){
                    ...
                    return $('#message').html('<span class="text-success text-center">'+ message +'</span>');
                }else{
                    const message = "@lang('translation.link_expire')";
                    return $('#message').html('<span class="text-danger text-center">'+ message +'</span>');
