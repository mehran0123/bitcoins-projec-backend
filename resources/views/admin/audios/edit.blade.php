...
<header class="page-title-bar">
  ...
  <div class="card">
    <!-- .card-body -->
    <div class="card-body">
      <h4 class="card-title">Edit Audio
    ...
    <form method="post" onsubmit="return false">
        ...
        <div class="form-row">
              ...
              @if(count($categories) > 0)
                <select name="category_id" class="form-control" id="category_id">
                  @foreach ($categories as $cat)
                    @if($cat->name !='Meditation Courses')
                    <option value="{{ $cat->id }}" @if($audio->category->id == $cat->id) {{ 'selected' }} @endif>{{ $cat->name }}</option>
                    @endif
                  @endforeach
                </select>
              @endif
          ...
          <div class="col-md-4 mb-3">
            ...
            <input type="text" class="form-control" value="{{ $audio->name }}" id="audio_name" name="audio_name" autofocus>
            <small id="audio_name_error" class="text-danger"></small>
        </div>
        <div class="col-md-4 mb-3">
            <label for="name">Artist</label>
            <input type="text" class="form-control" value="{{ $audio->artist }}" id="artist" name="artist" autofocus>
        ...
        <div class="row">
            ...
            <label for="name">Details</label>
                <textarea class="form-control" cols="5" rows="5" id="details">{{ $audio->details }}</textarea>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <audio controls>
                    <source src="{{ asset('/storage/app') }}/{{ $audio->audios->file_name }}" type="audio/mpeg">
                  </audio>
            </div>
        </div><br>
                ...
                <div class="group-contain" style="
                   ...
                   <div class="form-group" style="
                      ...
                      <div id="uploadList" class="list-group list-group-flush list-group-divider" style="margin: auto;">
                         <img src="{{ asset('/storage/app') }}/{{ $audio->image->file_name }}" alt="Preview Image" id="blah-one" height="100px">
                      </div>
                   </div>
                </div>
                <!-- /.form-group -->
     ...
     <hr>
      <button class="btn btn-primary btn-lg btn-block" type="button" id="edit_audio">Edit Audio</button>
    </form><!-- /form .needs-validation -->
    </div><!-- /.card-body -->
  </div>
    </div></div>
    @include('admin.audios.js.edit')
    @endsection
